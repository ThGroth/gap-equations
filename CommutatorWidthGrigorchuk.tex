...
We will now describe the procedure how to come from one equation to a new one. 
Any given equation uses only finitely many variables. We will start with an
oriented equation of class $\mathcal{O}_{n,0}$. Hence we can assume that
for each individual step the set $\mathcal{X}$ is of order $2n$ for some
$n\in\NN$ and fix the following free groups:

\begin{align*}
 F_{\mathcal X} &= \left<X_\ell \mid 1\leq \ell \leq 2n\right>,& 
 F_{\mathcal Y} &= \left<Y_{\ell,i} \mid \mid 1\leq \ell \leq 2n,\ i=1,2
 \right>.
\end{align*}
...
\begin{itemize}
\item[(*)] There is a generator $Y_{\gamma'}$ in $F_{\mathcal Y}$ and
  $x\in S$ with $\gamma'(Y_{\gamma'})=\pi(x)$, such that 
  $F_{\mathcal{Y}}$  factorizes as $F_{\mathcal Y \setminus Y_{\gamma'}} *
  \langle Y_{\gamma'} \rangle$ and the
  solvability of the constrained equation
  $(R_{2n-1} (g\at{2})^x \cdot g\at{1},\gamma'|_{F_{\mathcal Y \setminus Y_
  {\gamma'}}})$ implies
...
$(\gamma'*\pi)(Y_0)=({\gamma'*\pi})(w_2g_2w_1)$ so with
...
$\nf_{\gamma,n,Y_0}:=\nf_{v_1w_2g_2w_1v_1g_1}\colon F_{\mathcal
  Y'}*F_2 \to F_{\mathcal Y'}*F_2$ and note that
$\nf_{\gamma,n,Y_0}(\ell_{Y_0}(v))=R_{2n-1}g_2^{{Y_{\gamma'}}}g_1$ for
some generator ${Y_{\gamma'}}\in\mathcal{Y}$. 

!!!!Why $\gamma'$ here? shouldn't it be $\gamma$?

 This leads to the
following definition.
\[\Gamma_3^{q_1,q_2,n,Y_0}(\gamma) = \left\{\gamma'|_{\mathcal Y'} \circ \nf_
{\gamma,n,Y_0} \bigmid \gamma' \in \Gamma_2^{q_1,q_2,n} \right\}.\] 
...
!!!!$Y_{\gamma''}$ or $Y_{\gamma'}$

${Y_{\gamma''}}\mapsto \rep(\gamma''({Y_{\gamma''}}))$ to a solution
$s'$ of the equation
$(R_{2n-1} (g\at{2})^{{Y_{\gamma'}}} g\at{1},\gamma'')$. The map
$s'\circ \nf_{\gamma,n,Y_0}^{-1}$ is a solution for the constrained
equation
$(v_1w_2g_2w_1v_1g_1,\gamma'|_{\mathcal X'}:=\gamma'' \circ
\nf_{\gamma,n,Y_0}^{-1})$.  This can be extended by the mapping
$Y_0\mapsto w_2(g\at{2})w_1$ to a solution $s$ of
...
can be accessed in GAP as \gapinline{PCD.nonCommutatorGermGroup4}. For details
see Section \ref{sec:precomputation}.
...
\begin{description}
   ...
   \item [\texttt{``noncommutator''}] Inside the $4$-th level germgroup there is an element which is not
		      ...
		      element using the character table of the germgroup and the Burnside
          formula \eqref{eq:BurnsideFormula}. There are $3106$ irreducible
          characters and hence the search will
