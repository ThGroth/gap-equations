...
\end{lem}
 ...
 %TODO Ask Laurent if he knows a way to give more details here without repeating the 
%  same argument over and over again. Perhaps definition of \Phi_gamma already before this proposition?
\end{proof}
...
\begin{lem}[\cite{Lysenok:QudraticEquationsInGrig}]\label{lem:finitelyManyConstraints}
 ...
 there is an element $\varphi \in \Stab(R_n)<\Aut(F_{\mathcal{X}})$ such that $\supp(\gamma\circ\varphi) \in \left<X_1,\ldots,X_5\right>$.
\end{lem}
...
\begin{itemize}
 ...
 Denote generators $X_i\in\mathcal{X}$ such that $\supp(\gamma) \subset \left<X_1,\ldots,X_{2n}\right>$ and choose some 
 other $Y_1,\ldots,Y_{4n} \in\mathcal{X} \setminus \{X_1,\ldots,X_{2n}\}$. 
 Now define
 ...
 \begin{align*}
  %\Phi_{\gamma}\colon F_{\mathcal{X}}*F_1 &\to (F_{\mathcal{X}}*F_2)\wr C_2 \\ g&\mapsto \pair{g_1,g_2},\\ X_i\qquad &\mapsto \pair{X_i^{(1)},X_i^{(2)}}\act(\gamma(X_i)).
...
Take $q_1,q_2 \in Q,n\geq3\in \NN$ arbitrary and define
 ...
 By the following Lemma~\ref{lem:commonVar} there is 
...
%TODO:
%Expand this:
%There can't be a result for arbitrary subgroups, because the restricted product of all finite groups.
