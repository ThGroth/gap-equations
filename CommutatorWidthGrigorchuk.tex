...
to Section~\ref{sec:SelfSimilarGroups}.
...
group on the generating set $\mathcal{X}$. We use $\id$ for the
identity element of groups, and for the identity maps, to distinguish
it from the numerical $1$.
...
natural map $F_{\mathcal{X}}*G\to F_{\mathcal{X}}/[F_{\mathcal{X}},F_{
\mathcal{X}}]*\id$.
...
\begin{proof}[Proof of Proposition~\ref{pro:comwidthAutT2}]
 ...
 Thus the solvability of the constrained equation
...
% I tried to find the group K in the Paper Gri80 and Gri84 but I didn't find
% them. 
...
$\Stab(\eq E)$ denotes the group of $G$-auto\-mor\-phisms of $\eq E$.
...
\begin{lem}\label{lem:90Constraints}
 Identify the set $\{\gamma \colon F_{\mathcal{X}} \to Q \mid \supp(\gamma)
 \subset \left<X_1,\ldots, X_n\right>\}$ with $Q^{n}$. 
 Then 
...
\begin{enumerate}
 ...
 \item a group $Q_1 \subset Q \wr S_n$ such that 
 $\pair{q_1,\ldots,q_n}\sigma\in Q_1$ if
...
Let us write $k_1 := (ab)^2, k_2:=\pair{\id,k_1} = (abad)^2$ and $k_3:=\pair{k_1,\id} = (bada)^2$. Then 
 \begin{align*}
  ...
  &= \left<[k_2,k_1],[k_1,k_2^{-1}],[k_2,k_1]^{k_2},[k_1^{-1},k_2], [k_2,k_1]^{k_1},[k_2^{-1},k_1^{-1}]\right>^{\{\id,a\}} 
 % &= \left<(dacabaca)^2(baca)^4,((ca)^2baca)^2,(dacabaca)^2c(acab)^3acad,\right.\\
 % &\qquad\left.((ac)^3ab)^2, bacadacab(ac)^2(acab)^3, (acadacab)^2(acab)^4\right>^{\id,a}\ .
 \end{align*}
...
\begin{array}{rl}
 F_{\mathcal{X}}*F_{\mathcal G} &\to (F_{\mathcal{Y}}*F_{\mathcal{H}})\wr C_2,\\
  ...
  X_i &\mapsto \pair{Y_{i,1},Y_{i,2}}\act(\gamma(X_i)),
 \end{array}
...
\Gamma_2^{q_1,q_2,n}(\gamma),\varpi\colon \mathfrak{g}_i\mapsto q_i,Y_0.
...
Given $g\in G'$, $g_i = g\at{i}$ for $i=1,2$, an active constraint
$\gamma\in\Red_
{\act}$
...
%  \begin{align*}
%   p(g) &= w(ac,(ab)^2,\id,b) \cdot w(ca,\id,(ab)^2,b)\\ &\equiv w(ac,\id,\id,\id) \cdot w(ca,\id,\id,\id) \cdot w(\id,\id,\id,b)^2 \equiv \id \mod K.
%  \end{align*}
...
\begin{proof}[Proof of Proposition~\ref{pro:existsNextPair}]
 ...
 This last formula quantifies only over finite sets, and could be implemented.
 It can be checked in GAP with the function 
 \gapinline{verifyPropExistsSuccessor}. 
 \end{proof}
...
\begin{proof}
 ...
 $\gamma_1=(\id,\id,\pi(bad),\id)$, $\gamma_2=(\id,\id,\id,\pi(ca))$ are good pairs with active
...
details see Section \ref{sec:precomputation}.
 ...
 can be found in the file \filename{gap/precomputeNonCommutator.g}.
...
for each $g\in G'$ and hence the equation 
$a^{X_1}a^{X_2}a^{X_3}a^{X_4}a^{X_5}ag$.
...
the normalform of $\tilde \Phi_\gamma(a^{X_1}a^{X_2}a^{X_3}ag)$ turns out to
...
\begin{proof}
 ...
 \[T=\{\id,a,d^aa,d^a,b,ab^a,ca^d,bd^a\}.\]
...
\begin{proof}
 ...
 $\psi_0\colon K\to K\times K\le K$ be the map $k\mapsto\pair{k,\id}$
 and for $i\ge1$ let $\psi_i\colon K\to K\times K\le K$ be the map
 $k\mapsto\pair{\id,\psi_{i-1}(k)}$. Then
...
\item[\texttt{verifyCorollaryFiniteCWK}] Corollary~\ref{cor:KhasCW2} needs the
  existence of succeeding good pairs of the pair $(\id,\id)\in\RNK{K'}{K'}\times\Red^4$.
...
\begin{description}
   ...
   \item [\texttt{``goodpairs''}] First this will compute for each constraint $\gamma\in\Red\cup\Red^4$ 
          ...
          \eqref{def:Gammaq} which
           ...
           and is equipped with a progress bar. 
		      ...
		      Afterwards the succeeding pairs of $(\id,\id)$ which are 
   ...
   \item [\texttt{``conjugacywidth''}] Denote by $\eq{E}_g$ the equation $a^{X_1}a^{X_2}a^{X_3}a^{X_4}a^{X_5}ag$.
		      ...
		      $(\gamma * \pi)(\eq{E}_g) = \id$,
		      \[\eq{E}'_g := \nf(\tilde \Phi_\gamma(\eq{E}_g))=[X_1,X_2][X_3,X_4](g
          \at{2})(g\at{1}),\] and
   ...
   \item [\texttt{``noncommutator''}] Inside the $4$-th level germgroup there is
          ...
          This will compute the character table of 
          the $4$-th level germgroup and the set of irreducible characters the
          computation will take about $3$ hours. Then the characters are checked
          against Burnsides formula.
...
with $\gamma\in\Red$ and $X$ the decorated element $Y_{6,1}$ or $>Y_{4,1}$
depending if $\gamma\in\Red^4$ or $\gamma\in\Red$.
