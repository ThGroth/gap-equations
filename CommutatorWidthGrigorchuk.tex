...
Finite groups, and more generally virtually abelian groups, clearly have
...
$\Stab(\eq E)$ denotes group of $G$-automorphisms of $\eq E$.
...
\begin{itemize}
  \item there is a generator $Y_{\gamma'}$ in $F_{4n-1}$ and $x\in S$ such that $\gamma'(Y_{\gamma'})=\pi(x)$.
...
For $q_1,q_2 \in Q$ and $n\geq3\in \NN$ define
 ...
 \[(R_{2n-1}(g\at2)^{\textup{rep}(\gamma''({Y_{\gamma''}}))}(g\at1),\gamma'')\textup{
     for }\gamma'' \in \Gamma_3^{\pi(g\at{1}),\pi(g\at{2}),n,X_0}\]
 can be extended by the map
 ${Y_{\gamma''}}\mapsto \textup{rep}(\gamma''({Y_{\gamma''}}))$ to a
 ...
 we take the union of all of them and define
  ...
  by Lemma~\ref{lem:atIsWellDefinedModK'}.  Thus we can write
