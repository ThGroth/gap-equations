...
\replace{}{%
There are also known wide classes of groups of finite commutator width.
These include finitely generated virtually abelian-by-nilpotent groups 
(see \cite{Segal:Words}) and finitely generated solvable groups of class 3 \cite{Rhemtulla:Commutators}.
}
...
of the variables of $E$ then by passing to the states it leads to two new
...
\marginnote{\color{blue}Remark 1. See the end of the text}
\replace{}{
...
\begin{lem}\label{lem:90Constraints}
 ...
 Then \[\left|\RNK{Q^{2n}}{\Stab(R_n)}\right|=90 \text{ for all } n\geq 3	.\]
\end{lem}
\begin{proof}
\remove{
...
We need to prove the equality $|\Red_n| = 90$ for all $n \ge 3$.

Direct computation shows that $|\Red_3| = 90$ and \hl{$\Red_5 = \Red_4 = \Red_3$}, \marginnote{\color{blue} Remark 2}
...
\begin{enumerate}
 ...
 \item a group $Q_1 \subset Q \wr S_n$ such that $\pair{q_1,q_2}\sigma\in Q_1$ if
...
\begin{itemize}
 ...
 Now define
 \[\Gamma_1(\gamma) = \{\gamma' \colon \left< Y_1,\ldots ,Y_{4n}\right> \to Q \mid \pair{\gamma'(Y_{2i-1}),\gamma'(Y_{2i})}\in \omega^{-1}(\gamma(X_i)),i=1\ldots2n\}.\] 
...
For $q_1,q_2 \in Q$ and $n\geq3\in \NN$ define
 ...
 \hl{The map $\Gamma_3^{q_1,q_2,n,X_0}$ does not depend on $n$}: assume 
  \marginnote{\color{blue} Remark 3}
...
\marginnote{\color{blue} forward reference}
...
\marginnote{\color{blue} Remark 4}
...
an epimorphism $\germ_n\colon G\wr_{2^n} \RNK{G}{\Stab_G(n)}\to G_0\wr_{2^n} \RNK{G}{\Stab_G(n)}$.
...
been established before. Since we could not find an original reference we will
