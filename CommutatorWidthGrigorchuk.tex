...
This is then used to show that every subgroup of $G$ of finite index has
...
\begin{proof}
 ...
 $K^n$ have commutator width $2$. We start now with the case that $H\leq G$ is normal. 
 
 In %\cite TODO
 is shown that for $H\normal G$, $H\neq q$ there is $n$ such that $K^{2^n}\leq H$. As 
 $G$ is just infinite $[K^{2^n},K^{2^n}]$ has finite index in $[H,H]$.
 So if $T$ is a transversal of $\RNK{[H,H]}{[K^{2^n},K^{2^n}]}$ and 
 $N\in \NN$ such that every element in $T$ is a product of at most $N$ commutators
 in $H$ then $H$ has commutator width at most $N+2$.
 
 If $H$ is not necessarily normal we can use that $G$ has the congruence subgroup property
 \cite{Bartholdi:BranchGroups}%TODO not really. Better Grigorchuk:just infinite branch groups.
 to find a nontrivial normal subgroup $N<H$. By the same argument as before we can take a 
 finite transversal $T$ of $\RNK{[H,H]}{[N,N]}$ and find a number $M$ such that every element 
 of $T$ is a product of at most $M$ commutators in $H$. Then again if $M_1$ is the finite
 commutator width of $N$ we can write each element $h\in [H,H]$ as product $nt$, $n\in N$, 
 $t\in T$ which is thus a product of at most $M+M_1$ commutators. 
\end{proof}
