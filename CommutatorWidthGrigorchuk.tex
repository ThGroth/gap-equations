...
% that $\Phi_\gamma(R_n \mathfrak g)=\pair{v,w}\pair{\mathfrak g_1, \mathfrak
% g_2}$.  By Lemma~\ref{lem:commonVar} there is $Y_0 \in \mathcal{Y}\cup 
...
%     Y_i &\text{ if } Y_i\neq {Y_0} \\
%     w_2\mathfrak g_2w_1 &\text{ if }Y_i= {Y_0} 
%   \end{cases}\end{array}
% \]
% maps $v\mathfrak g_1$ to $v_1w_2\mathfrak g_2w_1v_2\mathfrak g_1$ and
% $w\mathfrak g_2$ to $\id$. For
% $\gamma'\in \Gamma_2^{q_1,q_2,n}(\gamma)$ we have
% $(\gamma'*\varpi)(Y_0)=({\gamma'*\varpi})(w_2\mathfrak g_2w_1)$. Thus all
% information of $\gamma'$ is already contained in its restriction to $F_
% {\mathcal{Y} \setminus \{Y_0,Y_0^{-1}\} }$.
% %  so with
...
% $\nf_{\gamma,n,Y_0}:=\nf_{v_1w_2\mathfrak g_2w_1v_2\mathfrak g_1}\colon F_{
% \mathcal{Y}\setminus
% \{Y_0,Y_0^{-1}\}}
%   *F_{\mathcal H} \to F_{\mathcal Y'}*F_{\mathcal H}$ and note that
%   \[ \nf_{\gamma,n,Y_0}(\ell_{Y_0}(v\mathfrak g_1))=R_{2n-1}(Y_{1,1},Y_
%   {1,2},\ldots,
%   Y_{2n-1,1},Y_{2n-1,2})\mathfrak g_2^{{Y_{2n,1}}}\mathfrak g_1. \] 
%  This leads to the
...
% Given $g\in G'$, $g_i = g\at{i}$ for $i=1,2$, a constraint $\gamma\colon
% F_{\mathcal{X}}\to Q$ and 
% $\gamma''\in \Gamma_3^{\pi(g_1),\pi(g_1),n,Y_0}(\gamma)$ then  a solution for
% the the constrained equation
% \[\mathcal{E}'=(R_{2n-1}(Y_{*,*})g_2^{\rep(\gamma''({Y_{2n,1}}))}
% g_1,\gamma'') \]
% can be extended by the map
% ${Y_{2n,1}}\mapsto \rep(\gamma''({Y_{2n,1}}))$ to a solution
% $s'$ of the equation
% $(R_{2n-1}(Y_{*,*}) (g_2)^{{Y_{2n,1}}} g_1,\gamma'')$. 
...
% {\gamma,n,Y_0}$
% maps $v_1w_2g_2w_1v_2g_1$ to $R_{2n-1}(Y_{*,*}) g_2^{{Y_{2n,1}}}g_1$

% Hence the map $s'\circ \mathfrak n$ is a solution for the constrained
% equation $(v_1w_2g_2w_1v_2g_1,\gamma'' \circ \mathfrak{n}|_{F_{\mathcal
% Y\setminus \{Y_0,Y_0^{-1}\}}})$. We
% extended this solution and constraint to $F_{\mathcal Y}$ by 
% \[s \colon Y_{i,j}\mapsto \begin{cases}
% w_2g_2w_1 \\
% s'\circ n (Y_{i,j})
% \end{cases}
% \gamma'\colon Y_{i,j}\mapsto \begin{cases}
% (\gamma'' \circ \mathfrak{n}|_{F_{\mathcal
% Y\setminus \{Y_0,Y_0^{-1}\}}})*\pi)(w_2g_2w_1) & \textup{ if } Y_
% {i,j}=Y_0\\
% \gamma'' \circ \mathfrak{n}|_{F_{\mathcal
% Y\setminus \{Y_0,Y_0^{-1}\}}}(Y_{i,j}) &\textup{ otherwise.}
% \end{cases}\]
%  %TODO Continue here: Phi_gamma defined on \mathfrak g ... 
% to obtain a solution $s$ of the constrained equation $\big((\id*i_{
% \mathcal{H}})\circ\Phi_\gamma(R_n\mathfrak g),\gamma'\big)$ and hence for 
...
% $3\leq m<n$ and choose fitting $v,w$ such that
% $\Phi_\gamma(R_m\mathfrak g)=\pair{v,w}\pair{\mathfrak g_1,\mathfrak g_2}$; then
% $\Phi_\gamma(R_n \mathfrak g)=\pair{v,w}\pair{R_{n-m}\mathfrak g_1,R_
% {n-m}'\mathfrak g_2}$, so after
% applying the homomorphism $\ell_{Y_0}$ the word which needs to be
% normalized is $v_1w_2R_{n-m}\mathfrak g_2w_1v_2R_{n-m}' \mathfrak g_1$. The
% automorphisms
% \begin{align*}
%   \psi_1\colon&\begin{array}{r@{\;}l}
%                  F_{\mathcal{X}}*F_{\mathcal{H}} &\to F_{\mathcal{X}}*F_
%                  {\mathcal H}\\
%                  Y &\mapsto Y^{\mathfrak g_1^{-1}}\\
%                  Z &\mapsto Z^{(\mathfrak g_2w_1v_1\mathfrak g_1)^{-1}}\\
%                  X &\mapsto X,
%               \end{array} &
%   \psi_2\colon&\begin{array}{r@{\;}ll}
%                  F_{\mathcal{X}}*F_{\mathcal{H}} &\to F_{\mathcal{X}}*F_
%                  {\mathcal{H}}\\
%                  Y &\mapsto Y^{\mathfrak g_2^{X_{11}}\mathfrak g_1}&\text{ for
%                  }Y\in \Var(R_{n-m}')\\
%                  Z &\mapsto Z^{\mathfrak g_2^{X_{11}}\mathfrak g_1}&\text{ for
%                  }Z\in \Var(R_{n-m})\\
%                  X &\mapsto X&\text{ for all other generators}
%                \end{array}
% \end{align*}
% have the property that
% $\nf_{v_1w_2R_{n-m}\mathfrak g_2w_1v_2R_{n-m}'\mathfrak g_1} =
...
\[\ell_{Y_0,n}\colon\begin{array}{r@{\;}l}
  ...
  \end{cases}\end{array}
\]
%maps $w R_{n-3}(Y_{7,2},\ldots,Y_{2n,2})\mathfrak g_2$ to $\id$.
...
%TODO give this property a name so I can refer to it below to explain why
% gamma''°n \in Gamma_2
% Thus all information of $\gamma'$ is already contained in its restriction to
% $F_{\mathcal{Y} \setminus \{Y_0,Y_0^{-1}\} }$.
...
\end{cases}
\]
% $s'\circ \mathfrak n$ is a solution for the constrained
% equation $(\mathcal{E},\gamma'' \circ \mathfrak{n}|_{F_{\mathcal
% Y\setminus \{Y_0,Y_0^{-1}\}}})$. We
% extended this solution and constraint to $F_{\mathcal Y}$ by 
% \[s \colon Y_{i,j}\mapsto \begin{cases}
% w_2g_2w_1 \\
...
\]
have the property that 
% \[\psi_2\circ\nf_{v_1w_2\mathfrak g_2w_1v_2\mathfrak g_1}\circ \psi_1(
% \mathcal{E})
% =R_{2n-1}(Y_{1,1},Y_{1,2},\ldots,Y_{5,1},Y_{5,2},Y_{7,2},Y_{8,2},\ldots,Y_
% {2n,2},Y_
% {7,1},Y_{8,1},\ldots,Y_{2n,1})\mathfrak{g}_2^{Y_{6,1}}\mathfrak{g}_1\]
...
those constraints that do not fulfill the requested properties; we
 ...
 \Gamma^q(\gamma) := \left\{\gamma' \in \Gamma_4^q(\gamma) \bigmid 
  ...
  \label{def:Gammaq}
 \end{align}
 %TODO Argh... dependency on n again!. Resolve this!
