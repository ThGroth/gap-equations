...
\begin{proof}
  ...
  generality the first case. Write further
...
\[\ell_{X_0}\colon\begin{array}{r@{\;}l}
...
\begin{proof}[Proof of Proposition~\ref{pro:existsNextPair}]
 In the construction above it is clear that the sets $\Gamma_3^{q,X_0}$ and hence $\Gamma_4^{q,X_0}$
...
can be accessed in GAP as \gapinline{PCD.nonCommutatorGermGroup4}.

!!!! A word on why this is not a commutator? Uses character table? Direct enumeration seems too long
...
\begin{description}
   ...
   \item [\texttt{``noncommutator''}] Inside the $4$-th level germgroup there is an element which is not
		      ...
		      element using the character table of the germgroup. The search will
